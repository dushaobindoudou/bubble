...
const addPlayer = (socket) => {
    ...
    socket.on('gotit', function (clientPlayerData) {
        ...
        if (map.players.findIndexByID(socket.id) > -1) {
            ...
            socket.emit('kick', '昵称格式无效，只能包含字母、数字和下划线');
            socket.disconnect();
        } else {
    ...
    socket.on('pass', async (data) => {
        ...
        if (password === config.adminPass) {
            ...
            socket.emit('serverMSG', '欢迎回来，管理员 ' + currentPlayer.name);
            socket.broadcast.emit('serverMSG', currentPlayer.name + ' 以管理员身份登录');
            currentPlayer.admin = true;
        } else {
            console.log('[ADMIN] ' + currentPlayer.name + ' attempted to log in with the incorrect password: ' + password);

            socket.emit('serverMSG', '密码错误，尝试已记录');
    ...
    socket.on('kick', (data) => {
        if (!currentPlayer.admin) {
            socket.emit('serverMSG', '你没有权限使用此命令');
            return;
        }
        ...
        for (let playerIndex in map.players.data) {
            ...
            if (player.name === data[0] && !player.admin && !worked) {
                ...
                socket.emit('serverMSG', '玩家 ' + player.name + ' 被 ' + currentPlayer.name + ' 踢出游戏');
        ...
        if (!worked) {
            socket.emit('serverMSG', '找不到该用户或用户是管理员');
        }
    });
...
const tickPlayer = (currentPlayer) => {
    if (currentPlayer.lastHeartbeat < new Date().getTime() - config.maxHeartbeatInterval) {
        sockets[currentPlayer.id].emit('kick', '心跳超时，已超过 ' + config.maxHeartbeatInterval + ' 毫秒未收到响应');
